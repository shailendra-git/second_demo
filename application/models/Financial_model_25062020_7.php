...
class Financial_model extends CI_Model
{
 ...
 //               INNER join `onelane_transaction_category` ON `onelane_transaction_category`.`id` =`onelane_transaction_data`.`category_id`".$where)->result_array();
    ...
    public function monthly_cron_fetch_data(){
                  //echo $owner_id;
        return $this->db->query("SELECT * FROM `onelane_transaction_data` WHERE  payment_type=2 AND DATE_FORMAT(`start_date`,'%Y-%m-%d') <= DATE_FORMAT(NOW(),'%Y-%m-%d') AND DATE_FORMAT(`end_date`,'%Y-%m-%d') >= DATE_FORMAT(NOW(),'%Y-%m-%d') AND `recurring_frequency` = 'monthly'  AND monthly_and_weekly = DATE_FORMAT(NOW(),'%d') AND owner_id=32")->result_array();
    ...
    public function weekly_cron_fetch_data(){
         ...
         return $this->db->query("SELECT * FROM `onelane_transaction_data` WHERE `payment_type`=2 AND DATE_FORMAT(`start_date`,'%Y-%m-%d') <= DATE_FORMAT(NOW(),'%Y-%m-%d') AND DATE_FORMAT(`end_date`,'%Y-%m-%d') >= DATE_FORMAT(NOW(),'%Y-%m-%d') AND `recurring_frequency`='weekly' AND monthly_and_weekly = LOWER(DAYNAME(NOW())) AND owner_id=32")->result_array();
