...
class Financial_model extends CI_Model
{
 ...
 //               INNER join `onelane_transaction_category` ON `onelane_transaction_category`.`id` =`onelane_transaction_data`.`category_id`".$where)->result_array();
	...
	public function saveDemo($data)
	{
        $this->db->insert('onelane_demo',$data);
	}
	public function saveDemoWeekly($data)
	{
        $this->db->insert('onelane_demo',$data);
	}
    ...
    public function monthly_cron_fetch_data(){
                  //echo $owner_id;
        return $this->db->query("SELECT * FROM `onelane_transaction_data` WHERE  payment_type=2 AND DATE_FORMAT(`start_date`,'%Y-%m-%d') <= DATE_FORMAT(NOW(),'%Y-%m-%d') AND DATE_FORMAT(`end_date`,'%Y-%m-%d') >= DATE_FORMAT(NOW(),'%Y-%m-%d') AND `recurring_frequency` = 'monthly'  AND monthly_and_weekly = DATE_FORMAT(NOW(),'%e') ")->result_array();
    ...
    public function weekly_cron_fetch_data(){
         ...
         return $this->db->query("SELECT * FROM `onelane_transaction_data` WHERE `payment_type`=2 AND DATE_FORMAT(`start_date`,'%Y-%m-%d') <= DATE_FORMAT(NOW(),'%Y-%m-%d') AND DATE_FORMAT(`end_date`,'%Y-%m-%d') >= DATE_FORMAT(NOW(),'%Y-%m-%d') AND `recurring_frequency`='weekly' AND monthly_and_weekly = LOWER(DAYNAME(NOW())) ")->result_array();
...
// 24-06-2020 create by 
    public function getAllTenantDataRecurring($propertyid)
	{
        
       return $this->db->query("SELECT `onelane_property_tenant`.`id`, `onelane_property_tenant`.`firstname` FROM `onelane_property_tenant` INNER JOIN `onelane_properties` ON `onelane_property_tenant`.`property_id` = `onelane_properties`.id WHERE `onelane_property_tenant`.`user_id` != 0 AND `onelane_property_tenant`.`property_id`=".$propertyid)->result_array();
       // echo $this->db->last_query();
	}

	 public function getAllTenantDataRecurringPropertyUnit($propertyid)
	{
        
       return $this->db->query("SELECT `onelane_property_tenant`.`id`, `onelane_property_tenant`.`firstname` FROM `onelane_property_tenant` INNER JOIN `onelane_properties` ON `onelane_property_tenant`.`property_id` = `onelane_properties`.id INNER JOIN `onelane_property_units` ON `onelane_property_units`.`id` = `onelane_property_tenant`.`unit_id` WHERE `onelane_property_tenant`.`user_id` != 0 AND `onelane_property_tenant`.`property_id`=".$propertyid)->result_array();
       // echo $this->db->last_query();
	}



	public function getAllTenantUnit($unit_id)
	{
        // echo $unit_d;
        // exit;
       return $this->db->query("SELECT `onelane_property_tenant`.`id`, `onelane_property_tenant`.`firstname` FROM `onelane_property_tenant` INNER JOIN `onelane_property_units` ON `onelane_property_tenant`.`unit_id` = `onelane_property_units`.id WHERE `onelane_property_tenant`.`user_id` != 0 AND `onelane_property_tenant`.unit_id=".$unit_id)->result_array();
       //  echo $this->db->last_query();
        // exit;
	}
    public function getAllTeamUnit($property_id)
	{
        // echo $unit_d;
        // exit;
       return $this->db->query("SELECT `onelane_users`.`firstname` FROM `onelane_users` INNER JOIN  `onelane_agent_property` ON `onelane_users`.`id`= `onelane_agent_property`.`agent_id` WHERE `onelane_agent_property`.`property_id`=".$property_id)->result_array();
       //  echo $this->db->last_query();
        // exit;
	}
	...
	public function gettenantdata($tenant_id){
		...
		return $this->db->query("SELECT `firstname` FROM `onelane_property_tenant` WHERE `id`=".$tenant_id)->row_array();

	}
			
	public function getteamdata($team_id){

		return $this->db->query("SELECT `firstname` FROM `onelane_users` WHERE `id`=".$team_id)->row_array();

	}

	public function cronWeeklyUserDetails(){
       
       return $this->db->query("select onelane_transaction_data.id,onelane_transaction_data.`owner_id`,CONCAT (onelane_users.firstname,' ',onelane_users.lastname) as fullname,onelane_users.email,onelane_amount_data.`p_amount` from onelane_transaction_data INNER JOIN onelane_amount_data ON onelane_transaction_data.id = onelane_amount_data.transaction_id INNER JOIN onelane_users ON onelane_users.id = onelane_transaction_data.owner_id where onelane_transaction_data.`created_at` between date_sub(now(),INTERVAL 1 WEEK) and now() GROUP BY onelane_users.email")->result_array();

	}

    public function getTranscationDataBetweenDates($owner_id,$start_date,$end_date){
		
          
        return $this->db->query("SELECT  `onelane_amount_data`.`p_amount`,onelane_transaction_data.money_type,onelane_amount_data.createdate , `onelane_properties`.`address`, onelane_property_units.unit,onelane_transaction_data.group_name,	onelane_transaction_data.category_id,onelane_transaction_data.payment_source,onelane_transaction_category.category_show_name  FROM `onelane_transaction_data` INNER JOIN `onelane_amount_data` ON `onelane_amount_data`.`transaction_id` = `onelane_transaction_data`.`id`
                                INNER JOIN `onelane_properties` ON `onelane_properties`.`id` = `onelane_transaction_data`.`property_all_id`
                                INNER JOIN `onelane_transaction_category` ON `onelane_transaction_category`.`id` = `onelane_transaction_data`.`category_id`
                                INNER JOIN `onelane_property_units` ON `onelane_property_units`.`id` = `onelane_transaction_data`.`unit_id` WHERE onelane_transaction_data.owner_id =".$owner_id." and DATE_FORMAT(onelane_amount_data.`createdate`,'%Y-%m-%d')  >= '".$start_date."' AND DATE_FORMAT(onelane_amount_data.`createdate`,'%Y-%m-%d')  <= '".$end_date."'  ORDER BY DATE_FORMAT(onelane_amount_data.`createdate`,'%Y-%m-%d')")->result_array();
      // echo $this->db->last_query();
      // exit;
	}
