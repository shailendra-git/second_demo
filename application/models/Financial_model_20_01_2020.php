...
class Financial_model extends CI_Model
{
 ...
 //               INNER join `onelane_transaction_category` ON `onelane_transaction_category`.`id` =`onelane_transaction_data`.`category_id`".$where)->result_array();
    ...
    public function monthly_cron_fetch_data(){
        ...
        return $this->db->query("SELECT * FROM `onelane_transaction_data` WHERE owner_id='".$this->owner_id."' AND payment_type=2 AND `start_date` <= NOW() AND `end_date` >= NOW() AND `recurring_frequency` = 'monthly'")->result_array();
           // echo $this->db->last_query();
           // exit;
    }
    ...
    public function weekly_cron_fetch_data(){
    	  ...
    	  return $this->db->query("SELECT * FROM `onelane_transaction_data` WHERE owner_id='".$this->owner_id."' AND `payment_type`=2 AND `start_date` <= NOW() AND `end_date` >= NOW() AND `recurring_frequency`='weekly'")->result_array();
    }
