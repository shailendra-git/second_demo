...

{
	...
	public function saveTenantData()
	{
        ...
        $category = 1;
        $money  = 'credit';
        $numofdate = substr($due_on,0,1);

        $transactionData =  array('payment_type'=>$payment_type,

                  'property_all_id'=>$property_id,
        ...
        $transaction_list = $this->financial_model->saveTenantTransactionData($transactionData);

         // $transaction = $this->financial_model->saveTenantTransactionPaidData(

         //          array('transaction_id'=>$transaction_last_id,

         //            'p_amount'=>$monthly_rent,
         //            'p_date'  =>$end_date,
         //            'p_memo' =>'new lease term',
         //            'p_payment_type' => 'recurring',
         //            'recurring_type' => 'monthly',
         //            'day_date_type' => $numofdate,
...
*/
	...
	public function tenants()
	{
	    ...
	    $data['gethistorytenantdata'] =  $this->team_model->getHistoryTenantData();
	...
	function getTenantDeleteById($tenantid){
		//echo $tenantid;
		//exit;

	    $getDeleteDatatenant = $this->team_model->getDeleteDatatenant($tenantid);

		$insertDeleteData = array(
                'old_tenant_id'     => $tenantid,
			    'property_id'       => $getDeleteDatatenant['property_id'],
			    'unit_id'           => $getDeleteDatatenant['unit_id'],
			    'owner_id'          => $getDeleteDatatenant['owner_id'], 
			    'user_id'           => $getDeleteDatatenant['user_id'],
			    'firstname'         => $getDeleteDatatenant['firstname'],
			    'lastname'          => $getDeleteDatatenant['lastname'], 
			    'mobile'            => $getDeleteDatatenant['mobile'],
			    'email'             => $getDeleteDatatenant['email'],
			    'address'           => $getDeleteDatatenant['address'],
			    'city'              => $getDeleteDatatenant['city'],
			    'state'             => $getDeleteDatatenant['state'], 
			    'zip'               => $getDeleteDatatenant['zip'], 
			    'start_date'        => $getDeleteDatatenant['start_date'],
			    'end_date'          => $getDeleteDatatenant['end_date'],
			    'after_lease_end'   => $getDeleteDatatenant['after_lease_end'],
			    'monthly_rent'      => $getDeleteDatatenant['monthly_rent'],
			    'due_on'            => $getDeleteDatatenant['due_on'],
			    'security_deposit'  => $getDeleteDatatenant['security_deposit'],
			    'late_fees_start_on'=> $getDeleteDatatenant['late_fees_start_on'],
			    'late_fee_type'     => $getDeleteDatatenant['late_fee_type'],
			    'late_fee_amount'   => $getDeleteDatatenant['late_fee_amount'],
			    'initial_amount'    => $getDeleteDatatenant['initial_amount'],
			    'daily_amount'      => $getDeleteDatatenant['daily_amount'],
			           
		);
		$getLastId =  $this->team_model->saveTenantHistory($insertDeleteData);
		$deleteTenant = $this->team_model->deleteTenantData($tenantid);
         
        //$gethistorytenantdata =  $this->team_model->getHistoryTenantData();
       // print_r($gethistorytenantdata);
		redirect(base_url('team/tenants'));

		//exit;
	}

	public function editViewHistoryTenantById($id)
	{	


		$data['getHistoryTenantUserId'] =  $this->team_model->getHistoryTenantUserId($id);
		...
		$this->load->view('edit_view_tenant_popup',$data);
	...
	public function resendEmailTenant($tenant_id){

    
	    $tenantData['tenantData'] = $this->team_model->tenantData($tenant_id);

	    if($tenantData['tenantData']['user_id'] == 0){
